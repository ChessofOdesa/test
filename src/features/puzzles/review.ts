...
export type PuzzleReviewResult = { frames: PuzzleFrame[]; explanation: string; score: string; depth: number | null };
...
export async function reviewPuzzle(attempt: Attempt, mistake: number | null, signal: AbortSignal): Promise<PuzzleReviewResult> {
    ...
    if (game.isGameOver()) return { frames: framesForLine(root, prefix), explanation: game.isCheckmate() ? 'Позиція завершилася матом.' : 'Цей хід завершує позицію нічиєю.', score: game.isCheckmate() ? 'Мат' : 'Нічия', depth: null };
    const result = await analyzeFenWithStockfish(game.fen(), 18, undefined, 12000, { signal, workerOnly: true, movetime: 3000, timeoutMs: 12000 });
    ...
    return { frames, explanation, score, depth: result.depth || null };
}
