...
export function RatingRange({ value, disabled, onChange, defaultOpen = false }: { value: PuzzleProgress['ratingRange']; disabled: boolean; onChange: (value: PuzzleProgress['ratingRange']) => void; defaultOpen?: boolean }) {
    ...
    return <details className="puzzle-range" open={expanded} onToggle={event => setExpanded(event.currentTarget.open)}><summary>{value ? `Точний діапазон: ${value.min}–${value.max}` : 'Точний діапазон'}</summary>
