...
export function PuzzleReview({ attempt, onPreview, actions, blocked = false }: { attempt: Attempt; onPreview: (frame: PuzzleFrame | null) => void; actions?: ReactNode; blocked?: boolean }) {
    ...
    return <section className="puzzle-review" aria-label="Розбір задачі">
        <div className="puzzle-engine-panel">
            <header className="puzzle-engine-header"><Button variant="ghost" className="puzzle-power" aria-label={enabled ? 'Вимкнути Stockfish' : 'Увімкнути Stockfish'} aria-pressed={enabled} onClick={() => setEnabled(!enabled)}><Power size={24} /></Button><div><h2>Stockfish</h2><small>{busy ? 'Розрахунок…' : !enabled ? 'На паузі' : review?.depth ? `Глибина ${review.depth}` : 'Розбір позиції'}</small></div><span className="puzzle-engine-score" title="Оцінка з боку білих">{line?.score || '—'}</span><Button variant="ghost" aria-label="Налаштування Stockfish" aria-expanded={settings} onClick={() => setSettings(!settings)}><SlidersHorizontal size={20} /></Button></header>
            ...
            {!!attempt.mistakes?.length && <div className="puzzle-review-choices"><Button disabled={!enabled} variant={selected === null ? 'secondary' : 'ghost'} onClick={() => setSelected(null)}>Ідея задачі</Button>{attempt.mistakes.map((mistake, index) => <Button key={`${mistake.step}-${mistake.move}`} disabled={!enabled} variant={selected === index ? 'secondary' : 'ghost'} onClick={() => setSelected(index)}>Помилка {framesForLine(attemptFen({ ...attempt, step: mistake.step }), [mistake.move])[1].san}</Button>)}</div>}
            ...
            {review?.lines.map((variant, row) => <div className="puzzle-engine-row" key={row} aria-label={`Варіант ${row + 1}`}><span title="Оцінка з боку білих">{variant.score}</span><div className="puzzle-review-line">{variant.frames.map((frame, index) => <button key={index} type="button" aria-pressed={row === lineIndex && ply === index} onClick={() => show(row, index)}>{frame.san}</button>)}</div></div>)}
            {review && <details className="puzzle-explanation"><summary>Пояснення та помилки</summary><p>{review.explanation}</p><small>{review.score}</small></details>}
        </div>
        <div className="puzzle-review-bottom">{actions}<nav className="puzzle-review-nav" aria-label="Перегляд варіанта"><Button variant="outline" aria-label="Попередній хід розбору" disabled={!line || ply === 0} onClick={() => step(-1)}>←</Button><span aria-live="polite">{ply ?? '—'} / {line ? line.frames.length - 1 : '—'}</span><Button variant="outline" aria-label="Наступний хід розбору" disabled={!line || ply === line.frames.length - 1} onClick={() => step(1)}>→</Button><Button variant="ghost" disabled={ply === null} onClick={() => { setPly(null); onPreview(null); }}>До розв’язання</Button><small>Клавіші ← →</small></nav></div>
    </section>;
}
