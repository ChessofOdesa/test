...
export function PuzzleReview({ attempt, rating, onPreview, actions, blocked = false }: { attempt: Attempt; rating?: number; onPreview: (frame: PuzzleFrame | null) => void; actions?: ReactNode; blocked?: boolean }) {
    ...
    return <section className="puzzle-review" aria-label="Розбір задачі">
        <div className="puzzle-review-summary">
            ...
            <div className="puzzle-best-line"><span>Розв’язання</span><p>{solution.slice(0, 6).join('  ')}{solution.length > 6 ? ' …' : ''}</p></div>
        ...
        <div className="puzzle-review-bottom"><nav className="puzzle-review-nav" aria-label="Перегляд варіанта"><Button variant="outline" title="Попередній хід розбору" aria-label="Попередній хід розбору" disabled={!line || ply === 0} onClick={() => step(-1)}>←</Button><span aria-live="polite">{ply ?? '—'} / {line ? line.frames.length - 1 : '—'}</span><Button variant="outline" title="Наступний хід розбору" aria-label="Наступний хід розбору" disabled={!line || ply === line.frames.length - 1} onClick={() => step(1)}>→</Button><Button variant="ghost" disabled={ply === null} onClick={() => { setPly(null); onPreview(null); }}>До розв’язання</Button><small>Клавіші ← →</small></nav>{actions}</div>
    </section>;
}
