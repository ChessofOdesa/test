...
export function PlayerBar({ player, lowTimeSound }: {
    ...
    <div className="room-player-copy"><div className="room-player-name"><strong>{player.name}</strong>{player.rating != null && <span>{player.rating}</span>}{player.ratingChange != null && <span className={player.ratingChange < 0 ? "rating-down" : "rating-up"}>{player.ratingChange > 0 ? "+" : ""}{player.ratingChange}</span>}</div>
      <div className="room-player-detail"><span className={`room-color ${player.color === "w" ? "is-white" : "is-black"}`} aria-hidden="true"/><span>{player.subtitle || (player.color === "w" ? "Білі" : "Чорні")}</span>{typeof player.connected === "boolean" && <span className={`room-presence ${player.connected ? "is-connected" : ""}`} title={player.connected ? "Підключено" : "З’єднання втрачено"} aria-label={player.connected ? "Підключено" : "З’єднання втрачено"}/>}
    ...
    </div></div><ChessClock clock={player.clock} name={player.name} lowTimeSound={lowTimeSound && player.own}/>
  </section>;
}
