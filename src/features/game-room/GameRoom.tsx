...
import { roomBoardSize } from "./rules";
...
export function GameRoom({ mode, title, timeControl, board, top, bottom, preferences, updatePreferences, onFlip, evaluation, boardNotice, ...panel }: Props) {
    ...
    useEffect(() => {
        const resize = () => { const width = root.current?.clientWidth || window.innerWidth; const header = document.fullscreenElement ? 0 : document.querySelector(".app-header")?.getBoundingClientRect().height || 60; setSize(roomBoardSize(width, window.innerHeight, header)); };
        resize();
    ...
    return <div ref={root} className={`game-room-page ${preferences.focus ? "is-focused" : ""}`} style={{ "--room-board-size": `${size}px` } as CSSProperties}>
