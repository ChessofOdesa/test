...
import { OnlinePlayIcon } from "@/components/icons/chess";
...
export function OnlinePlayPanel({ preferences, update, onStart, ratedAvailable, casualAvailable = true, disabled, custom = false, enhanced = true }: { preferences: PlayPreferences; update: (p: Partial<PlayPreferences>) => void; onStart: () => void; ratedAvailable: boolean; casualAvailable?: boolean; disabled: boolean; custom?: boolean; enhanced?: boolean }) {
  return <><div className="play-panel-heading"><span className="play-section-icon" aria-hidden="true">{custom ? <SlidersHorizontal size={24}/> : <OnlinePlayIcon size={24}/>}</span><div><h2>{custom ? "Своя гра" : "Швидка гра"}</h2><p>{custom ? "Налаштуйте партію під свій темп." : "Оберіть час — ми знайдемо суперника."}</p></div></div>
    ...
    <Button className="play-primary-action" onClick={onStart} disabled={disabled}>{custom ? <SlidersHorizontal size={20} aria-hidden="true"/> : <OnlinePlayIcon size={20} aria-hidden="true"/>}{custom ? "Створити гру" : "Знайти суперника"}</Button>
  </>;
}
