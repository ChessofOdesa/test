import analyzeFenWithStockfish, { normalizeSideToMoveResultForWhite, type AnalyzeResult, } from "@/lib/stockfish";
...
describe("Lichess cloud evaluation", () => {
    it("uses the configured server endpoint and converts variations", async () => {
        ...
        const result = await analyzeFenWithStockfish(START_FEN, 15, undefined, 5000, {
            multiPv: 2,
            preferCloud: true,
        });
        ...
        expect(result.nodes).toBe(1234000);
        expect(result.lines).toHaveLength(2);
        expect(result.lines?.[1].scoreMate).toBe(6);
        const requestedUrl = new URL(requestedUrlString);
        expect(requestedUrl.origin + requestedUrl.pathname).toBe("https://server.example/api/evaluation");
    ...
    it("reports engine failure without inventing a Stockfish evaluation", async () => {
        vi.stubGlobal("Worker", class {
            constructor() { throw new Error("Worker unavailable"); }
        });
        vi.stubEnv("VITE_STOCKFISH_NATIVE_URL", "");
        await expect(analyzeFenWithStockfish(START_FEN, 8)).rejects.toThrow("Stockfish недоступний");
    });
});
