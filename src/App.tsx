...
const OnlineRoutes = lazy(() => import("./app/OnlineRoutes"));
...
function AppShell() {
    ...
    <main id="main-content" className="app-main"><RouteBoundary key={/^\/(online|game)(\/|$)/.test(location.pathname) ? "online" : location.pathname}><Suspense fallback={<LoadingPage />}><Routes>
      <Route path="/" element={<Index />}/><Route path="/play" element={<PlayHub />}/><Route path="/play/computer" element={<Play />}/>
      <Route path="/online" element={<RequireAccount><OnlineRoutes /></RequireAccount>}/><Route path="/game/:gameId" element={<RequireAccount><OnlineRoutes /></RequireAccount>}/>
...
export default function App() { return <QueryClientProvider client={queryClient}><AuthProvider><TooltipProvider><BoardSettingsProvider><Toaster /><Sonner /><BrowserRouter><AppShell /></BrowserRouter></BoardSettingsProvider></TooltipProvider></AuthProvider></QueryClientProvider>; }
