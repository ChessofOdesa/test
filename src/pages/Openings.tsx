...
export default function Openings() {
    ...
    const selectedOpeningProgress = selectedOpening
        ? growthState.openingProgress.find((item) => item.openingName === selectedOpening.name && item.lineName === (selectedLine?.name || "Main line"))
        : null;
    ...
    return (<div className="min-h-screen py-8">
      <div className="container">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="mb-6 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-border bg-gradient-card p-4 shadow-card">
              <p className="text-sm font-body uppercase tracking-[0.18em] text-muted-foreground">Прогрес тренування</p>
              <p className="mt-2 text-2xl font-display font-bold text-foreground">{totalCompletedDrills}</p>
              <p className="text-xs text-muted-foreground">завершених тренувань із {totalOpeningAttempts}  спроб</p>
            </div>
            <div className="rounded-xl border border-border bg-gradient-card p-4 shadow-card">
              <p className="text-sm font-body uppercase tracking-[0.18em] text-muted-foreground">Поточний варіант</p>
              ...
              <p className="text-xs text-muted-foreground">успішних тренувань / спроб у цьому дебюті</p>
            </div>
            <div className="rounded-xl border border-border bg-gradient-card p-4 shadow-card">
              <p className="text-sm font-body uppercase tracking-[0.18em] text-muted-foreground">Із розбору партій</p>
              <p className="mt-2 text-lg font-display font-bold text-foreground">{openingNotebookMistakes.length}  виправлень у дебюті</p>
        ...
        ].map(f => (<button key={f.key} onClick={() => setFilter(f.key)} className={`px-3 py-1.5 rounded-lg text-xs font-body font-medium transition-all ${filter === f.key
          ...
          <div className="grid lg:grid-cols-[1fr_auto_1fr] gap-6 items-start">
            ...
            return (<motion.div key={opening.name} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`bg-gradient-card rounded-xl p-4 border cursor-pointer transition-all ${isSelected ? "border-gold/40 shadow-gold" : "border-border hover:border-gold/20"}`} onClick={() => startTrainer(opening)}>
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h3 className="font-display font-semibold text-foreground text-sm">{opening.name}</h3>
                        <span className="text-sm text-muted-foreground font-body">{opening.eco}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <StyleIcon size={12} className="text-primary"/>
                        <span className="text-sm text-muted-foreground font-body">{STYLE_LABELS[opening.style]}</span>
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground font-body mb-2 line-clamp-2">{opening.description}</p>
                    <div className="flex items-center gap-3">
                      <div className="flex gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (<Star key={i} size={10} className={i < opening.popularity ? "text-primary fill-primary" : "text-muted-foreground/30"}/>))}
                      </div>
                    ...
                    {/* Lines */}
                    <AnimatePresence>
                      {isSelected && opening.lines.length > 0 && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-3 pt-3 border-t border-border space-y-1.5 overflow-hidden">
                          <span className="text-sm text-muted-foreground font-body uppercase tracking-wider">Варіанти</span>
                          {opening.lines.map((line) => (<button key={line.name} onClick={(e) => { e.stopPropagation(); startTrainer(opening, line); }} className={`w-full text-left flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-body transition-all ${selectedLine?.name === line.name
                            ? "bg-primary/10 text-primary"
                            : "text-secondary-foreground hover:bg-secondary"}`}>
                              <span>{line.name}</span>
                              <div className="flex items-center gap-1.5">
                                <span className="text-sm text-muted-foreground">{line.evaluation}</span>
            ...
            <div className="flex flex-col items-center gap-3">
              {selectedOpening ? (<>
                  ...
                  <ChessBoard key={trainerKey} initialFen={trainerGame.fen()} size={Math.min(420, typeof window !== "undefined" ? window.innerWidth - 48 : 420)} onMove={handleTrainerMove} interactive={!isComplete && trainerStep % 2 === 0}/>
                  ...
                  {/* Expected move hint */}
                  {!isComplete && (<div className="bg-gradient-card rounded-lg p-3 border border-border text-center max-w-[420px]">
              ...
              {selectedOpening && (<>
                  <div className="bg-gradient-card rounded-xl p-5 border border-border shadow-card">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className="text-sm px-2 py-0.5 rounded-full bg-primary/10 text-primary font-body">{STYLE_LABELS[selectedOpening.style]}</span>
                      <span className="text-sm px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground font-body">{selectedOpening.eco}</span>
                    </div>
                  </div>

                  {selectedLine && (<div className="bg-gradient-card rounded-xl p-5 border border-border shadow-card">
                  ...
                  <div className="bg-gradient-card rounded-xl p-5 border border-border shadow-card">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {currentMoves.map((move, i) => (<span key={i} className={`text-xs px-2 py-1 rounded font-body ${i < trainerStep
