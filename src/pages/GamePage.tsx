...
import { ArrowLeft, Ban, Copy, Download, Flag, Handshake, RefreshCw, Search, Swords } from "lucide-react";
...
export default function GamePage() {
    ...
    const result = ended ? { ...resultPresentation(liveGame.result, color, liveGame.reason || ""), result: liveGame.result, rating, actions: <>{rematchPossible && <Button disabled={online.rematchRequested} onClick={() => online.requestRematch(liveGame.timeControl, liveGame.id!)}><RefreshCw size={15}/>{online.rematchRequested ? "Запропоновано…" : "Реванш"}</Button>}<Button variant={rematchPossible ? "outline" : "default"} onClick={newGame}><Swords size={15}/>Нова партія</Button><Button variant="outline" disabled={!history.moves.length} onClick={() => navigate("/analysis", { state: { gameId: liveGame.id, pgn: liveGame.pgn } })}><Search size={15}/>Аналіз</Button><Button asChild variant="ghost"><Link to="/play"><ArrowLeft size={15}/>Назад до Грати</Link></Button></> } : undefined;
