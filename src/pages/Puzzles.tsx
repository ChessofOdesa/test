    ...
    return <div className="puzzles-studio">
        ...
        <section className="puzzle-board-card puzzle-card" aria-label="Дошка задачі">
            ...
            <div ref={boardWrap} className={`puzzle-board-wrap${loading ? ' is-loading-next' : ''}${reviewReady ? ' is-solved' : ''}`}>
                ...
                {bestMove && attempt?.hintLevel && !complete && <div className="puzzle-mark-layer" style={{ width: boardSize, height: boardSize }} aria-hidden="true"><span key={`${attempt.step}-${attempt.hintLevel}-from`} className="puzzle-hint-focus" style={squarePosition(bestMove.slice(0, 2), boardBlack)} />{attempt.hintLevel === 2 && <span key={`${attempt.step}-to`} className="puzzle-hint-focus is-target" style={squarePosition(bestMove.slice(2, 4), boardBlack)} />}</div>}
            </div>
