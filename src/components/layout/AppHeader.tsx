...
import { KnightIcon } from "@/components/icons/chess";
export function AppHeader() {
    ...
    <div className="header-inner !min-h-[58px] !gap-4 !px-5">
      <Link to="/" className="brand !gap-2 !text-[.98rem]" aria-label="Chess of Odesa — головна">
        <span className="brand-mark !h-8 !w-8 !rounded-lg" aria-hidden="true"><KnightIcon size={23} /></span>
        <span>Chess of <strong>Odesa</strong></span>
      </Link>
      <nav aria-label="Головна навігація" className="primary-nav !gap-1">
        {primaryNavigation.map(({ label, path, icon: Icon, matches }) => <Link key={path} to={path} aria-current={isNavigationActive(pathname, matches) ? "page" : undefined} className="nav-link !gap-1.5 !px-3 !py-2 !text-sm"><Icon size={18} aria-hidden="true"/><span>{label}</span></Link>)}
      </nav>
      <div className="header-actions !gap-1">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            {secondaryNavigation.map(({ label, path, icon: Icon }) => <DropdownMenuItem asChild key={path}><Link to={path}><Icon size={18} aria-hidden="true" className="mr-2" />{label}</Link></DropdownMenuItem>)}
